...
def load_recipe(data_path):
    fname = "{path}recipe.yaml".format(path=data_path)
    f = open(fname, 'r')
    return yaml.load(f)
...
def reduce_history(table):
    ...
    for uid, history in table.items():
        try:
            reduced[uid] = mean(history[1])
        except Exception, e:
            pass
            # found item with no history??

    return reduced


def consolidate_history(questions, network_function, verbose=False):
    return reduce_history(build_history_table(questions, network_function, verbose=verbose))
